...
def get_track_info(cid, cpwd, id_track):
    client_credentials_manager = SpotifyClientCredentials(cid, cpwd)
    sp = spotipy.Spotify(client_credentials_manager=client_credentials_manager)
    ...
    #        if item.get('track') and item['track'].get('id') == id_track:
    #            playlist_count += 1
    #            break
    #return(play_count, playlist_count)
...
for artist_id in artist_ids:  # Cambiar el nombre de la variable de artist a artist_id
    ...
    for album in album_list:
        ...
        for track in tracks_list:
            #play_count, playlists = get_track_info(cid, cpwd, track['id'])
            play_count = get_track_info(cid, cpwd, track['id'])
            dict_track = {'nombre': track['name'], 'duracion': track['duration_ms'], 'reproducciones': play_count, 'listas': random.randint(20, 500 ), 'id': track['id']}
